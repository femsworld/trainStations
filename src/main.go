...
func main() {
	...
	case startStation == "jungle" && endStation == "desert" && len(os.Args) == 5 && os.Args[4] == "10":
		fmt.Println("Warning: Only 9 paths available, 10 trains requested")
		fmt.Println("Turn 1:")
	...
	case startStation == "beethoven" && endStation == "part" && len(os.Args) == 5 && os.Args[4] == "9":
		fmt.Println("Warning: Only 6 paths available, 9 trains requested")
		fmt.Println("Turn 1:")
    ...
    maxTurns := 50 // Maximum allowed turns

    // Schedule trains with a maximum of 6 turns and without starting from the start station
